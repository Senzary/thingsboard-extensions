...
function BulkImportAssetController(instance) {
    ...
    const templatesObservable = fetchGroupByName(
        ...
        rxjs.map((assetInfo) => {
            ...
            assetInfo.optionLabel = templateName.charAt(0).toUpperCase()
            assetInfo.optionLabel += templateName.slice(1);
            return assetInfo;
        }),
        rxjs.toArray(),
    ...
    const templateSubscription = vm.bulkImportFormGroup
        ...
        .subscribe((value) => {
            ...
            const templateFieldsSubscription = getEntityAttributes(
                ...
                vm.thresholdFields = parseTemplateThresholds(latestAttributes);
            ...
            return createAssetObservable.pipe(
                rxjs.switchMap((newAsset) => {
                    ...
                    const locationToCompanyObservable = entityRelationService.saveRelation({
                        from: vmInstance.customer.entityId,
                        to: newAsset.id,
